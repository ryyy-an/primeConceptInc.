...
document.addEventListener("DOMContentLoaded", function () {
  ...
  window.filterInventory = function () {
    ...
    cards.forEach((card) => {
      ...
      const whText = card.querySelector(".bg-blue-50").textContent;
      const srText = card.querySelector(".bg-orange-50").textContent;

      const whStock = parseInt(whText.replace(/[^\d]/g, "")) || 0;
      const srStock = parseInt(srText.replace(/[^\d]/g, "")) || 0;
  ...
  // Add change tracker for edit form
...
// This will be called by your custom dropdown buttons
...
function selectFilter(value) {
  ...
  // Update visual feedback (Optional: you can add a checkmark to the active one)
...
window.addComponentRow = function (containerId = "componentsContainer") {
  ...
  const html = `
        <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-2xl group border border-transparent hover:border-gray-200 transition-all">
            ...
            <button type="button" onclick="confirmRemoveRow(this, 'component')" class="text-gray-300 hover:text-red-500 transition-colors">
        ...
        <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-2xl group border border-transparent hover:border-gray-200 transition-all">
            <div class="w-14 h-14 bg-white rounded-xl border border-gray-200 flex items-center justify-center shrink-0 relative">
                ...
                <label class="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex items-center justify-center cursor-pointer rounded-xl transition-all">
                    <input type="file" name="variant_imgs[]" accept=".jpg,.jpeg,.png" class="hidden" onchange="previewVariantImage(this, ${index})" required>
            ...
            <button type="button" onclick="confirmRemoveRow(this, 'variant')" class="text-gray-300 hover:text-red-500 transition-colors">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"></path>
                </svg>
            </button>
...
window.saveNewProduct = async function () {
  ...
  // Required fields validation using built-in HTML5 reportValidity
  ...
  try {
    ...
    if (result && result.success) {
      ...
      if (typeof window.showCustomAlert === "function") {
        window.showCustomAlert("Error: " + errorMsg);
      } else {
    ...
    if (result && result.success) {
      ...
      setTimeout(() => { window.formHasUnsavedChanges = false; }, 50);
        ...
        if (typeof window.showCustomAlert === "function") {
            ...
            alert("Error: " + (result.message || "Failed to fetch details."));
        }
    }
  } catch (error) {
    console.error(error);
...
// Logic moved to global.js for system-wide reuse
...
window.handleSaleToggle = async function(checkbox) {
    ...
    try {
        ...
        if (result && result.success) {
            if (typeof window.showCustomSuccess === "function") {
                window.showCustomSuccess("Sale status updated in database!");
            }
        } else {
            checkbox.checked = !checkbox.checked; // revert
            toggleEditSaleDiscount(checkbox);
            if (typeof window.showCustomAlert === "function") {
                window.showCustomAlert("Error updating sale status: " + (result.message || "Unknown error"));
            } else {
                alert("Error updating sale status: " + (result.message || "Unknown error"));
            }
        }
    } catch (e) {
        checkbox.checked = !checkbox.checked; // revert
        toggleEditSaleDiscount(checkbox);
        if (typeof window.showCustomAlert === "function") {
            window.showCustomAlert("Server error connecting to database.");
        } else {
...
window.addEditComponentRow = function(id = '', name = '', loc = '', qty = '') {
    ...
    // Auto-disable if existing data is being populated; leave enabled if new row
...
window.addEditVariantRow = function(id = '', name = '', image = 'default.png', lowStock = '10', overrideIndex = null) {
  ...
  const isExisting = name !== '';
  const disabledAttr = isExisting ? 'disabled' : '';
  const penColor = isExisting ? 'text-gray-300' : 'text-blue-500'; // Actually target uses green for variants sometimes, but blue is consistent with the toggle behavior
  ...
  const html = `
        <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-2xl group border border-transparent hover:border-gray-200 transition-all">
            ...
            <div class="w-14 h-14 bg-white rounded-xl border border-gray-200 flex items-center justify-center shrink-0 relative">
                ...
                <label class="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 flex items-center justify-center cursor-pointer rounded-xl transition-all">
                    <input type="file" name="variant_imgs[]" accept=".jpg,.jpeg,.png" class="hidden" onchange="previewEditVariantImage(this, ${index})" ${id === '' ? 'required' : ''}>
                    <svg class="w-3 h-3 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path d="M12 4v16m8-8H4" stroke-width="2" stroke-linecap="round"></path>
                    </svg>
                </label>
...
window.handleEditSave = function () {
  ...
  // Required fields validation using built-in HTML5 reportValidity
  // We temporarily enable disabled fields so validation can check them
  const disabledElements = form.querySelectorAll(':disabled');
  disabledElements.forEach(el => el.disabled = false);

  if (!form.reportValidity()) {
      // Re-disable elements if validation fails
      disabledElements.forEach(el => el.disabled = true);
      return;
  }

  // Re-disable elements after validation
  disabledElements.forEach(el => el.disabled = true);

  // Requirement: Only trigger confirmation modal if NO changes were made
  if (!window.formHasUnsavedChanges) {
    ...
    // If changes were made, proceed directly to save
...
window.executeEditSave = async function () {
  ...
  try {
    ...
    if (result && result.success) {
        // Requirement: Toast should appear ONCE modals disappear.
        // Close both modals before showing toast.
        if (typeof window.closeModal === "function") {
            window.closeModal('editConfirmModal');
        ...
        if (typeof window.showCustomSuccess === "function") {
            window.showCustomSuccess("Product has been successfully edited", () => {
                window.location.reload();
            });
        } else {
    ...
    if (typeof window.showCustomAlert === "function") {
        window.showCustomAlert("An error occurred while updating.");
    } else {
...
window.toggleSaleDatabaseCheckbox = async function (productCode, checkbox) {
  ...
  try {
    ...
    if (result && result.success) {
      ...
      alert(
        "Error updating sale status: " + (result.message || "Unknown error"),
      );
    }
  } catch (e) {
    // Revert UI toggle on failure
...
window.toggleSaleDatabase = async function (productCode, btn) {
  // Current status stored in data attribute
  const currentStatus = parseInt(btn.getAttribute("data-status")) || 0;
  ...
  try {
    ...
    if (result && result.success) {
      ...
      const dot = btn.querySelector(".status-indicator");
      const text = btn.querySelector(".btn-text");

      if (newStatus === 1) {
        btn.className =
          "w-full py-3 px-4 bg-orange-500 text-white font-bold rounded-xl text-xs hover:bg-orange-600 shadow-md shadow-orange-200 transition-all uppercase tracking-widest flex justify-center items-center gap-2";
        dot.className =
          "status-indicator w-2 h-2 rounded-full bg-white animate-pulse";
        text.textContent = "Turn Sale OFF";
        document.getElementById("discountInput").disabled = false;
      } else {
        btn.className =
          "w-full py-3 px-4 bg-gray-200 text-gray-600 font-bold rounded-xl text-xs hover:bg-gray-300 transition-all uppercase tracking-widest flex justify-center items-center gap-2";
        dot.className = "status-indicator w-2 h-2 rounded-full bg-gray-400";
        text.textContent = "Turn Sale ON";
        document.getElementById("discountInput").disabled = true;
      }

      if (typeof window.showCustomSuccess === "function") {
        window.showCustomSuccess("Sale status updated successfully!");
      }
    } else {
      alert(
        "Error updating sale status: " + (result.message || "Unknown error"),
...
window.toggleEditRow = function (button) {
  // Find the parent container of the row
  const row = button.closest(".group");
  if (!row) return;

  // Find all standard inputs inside that row but EXCLUDE hidden ones and file ones!
  ...
  if (isCurrentlyDisabled) {
    // Was disabled, now enabled -> focus the first input so user can type immediately
    inputs[0].focus();
...
window.openStockModal = async function(code) {
  ...
  try {
    ...
    const result = await response.json();
    if (result && result.success) {
      populateStockModal(result.product);
      openModal('stockModal');
    } else {
      if (typeof window.showCustomAlert === "function") {
          window.showCustomAlert("Error: " + (result.message || "Failed to fetch stock details."));
      } else {
          alert("Error: " + (result.message || "Failed to fetch stock details."));
      }
    }
  } catch (error) {
    console.error(error);
...
window.populateStockModal = function(p) {
                        ...
                        <div class="flex items-center mb-4">
                            ...
                            <div class="flex flex-col items-center min-w-[150px]">
                                ...
                                <div class="flex items-center bg-white border border-gray-200 rounded-xl p-0.5 w-full">
                                    <button onclick="stepDownValue('wh-v-${v.variant_id}')"
    ...
    // Showroom Content (Fixed relative adjustment logic)
                    ...
                    <div class="p-4 bg-white border border-gray-100 rounded-3xl flex items-center gap-5 shadow-sm">
                        ...
                        <div class="flex flex-col items-center">
                            ...
                            <div class="flex items-center bg-white border border-gray-200 rounded-xl p-0.5 gap-0.5">
                                <button onclick="stepDownValue('sr-v-${v.variant_id}')"
                                ...
                                <button onclick="stepUpValue('sr-v-${v.variant_id}')"
...
window.syncInventory = function () {
  ...
  variantInputs.forEach(input => {
      ...
      if (!card) return; // Prevent crash if variant-card is missing (e.g. Showroom tab)
      
      const recipeLabels = card.querySelectorAll('.live-balance');

      // Update Total Balance Logic (Existing)
...
window.stepUpValue = function (id) {
  ...
  if (input) {
    input.stepUp();
    if (input.classList.contains('variant-input')) syncInventory();
  }
};
...
window.handleStockUpdate = function() {
    ...
    const adjustments = [];
    const variantStats = {}; // To track net warehouse changes per variant
    let hasReduction = false;

    // First Pass: Aggregate variant changes for validation
    for (const input of inputs) {
        const diff = parseInt(input.value) || 0;
        const type = input.getAttribute('data-type');
        const variantId = input.getAttribute('data-id');

        if (type === 'WH_VAR' || type === 'SR_VAR') {
            if (!variantStats[variantId]) {
                ...
                // Try to find the variant name for better error message
                const card = input.closest('.p-4') || input.closest('.variant-card');
                if (card) {
                    const nameEl = card.querySelector('h4');
                    if (nameEl) variantStats[variantId].name = nameEl.textContent.trim();
                }
            }
    ...
    for (const input of inputs) {
        ...
        if (diff !== 0) {
            // Hard Validation: Showroom/Component stock cannot go below 0 (Direct check)
    ...
    if (adjustments.length === 0) {
        if (typeof window.showCustomAlert === "function") {
            window.showCustomAlert("No adjustments made.");
        } else {
            alert("No adjustments made.");
        }
        return;
    }

    // Store globally for executeStockUpdate
...
window.executeStockUpdate = async function() {
    ...
    // Use specific ID for the button to provide feedback
        ...
        if (result && result.success) {
            if (typeof window.showCustomSuccess === "function") {
             ...
             if (typeof window.showCustomAlert === "function") {
                ...
                alert("Error: " + (result.message || "Failed to update stocks."));
            }
        }
    } catch (error) {
        console.error(error);
        if (typeof window.showCustomAlert === "function") {
            window.showCustomAlert("A system error occurred. Check console.");
        }
    } finally {
    ...
    if (searchInput) {
        searchInput.addEventListener('input', function(e) {
            ...
            cards.forEach(card => {
                ...
                const code = card.querySelector('h2').innerText.toLowerCase();
...
// 3. Close filter menu when clicking outside
document.addEventListener('click', function(e) {
    ...
    if (filterMenu && !filterMenu.classList.contains('hidden')) {
        ...
        if (!filterMenu.contains(e.target) && !filterBtn) {
            filterMenu.classList.add('hidden');
        }
    }
});

// 4. Location Filtering Logic
window.selectFilter = function(type) {
    // 1. Close the menu
    const menu = document.getElementById('filterMenu');
    if (menu) menu.classList.add('hidden');

    // 2. Select all relevant elements
    ...
    overallQtyElements.forEach(el => {
        ...
        if (type === 'warehouse') {
            ...
            // Show only WH tags in this card
            ...
            // Show only SR tags in this card
